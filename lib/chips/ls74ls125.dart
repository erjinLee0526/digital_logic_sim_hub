...
/// 74LS125 - Quad Bus Buffer with 3-State Outputs, Active-Low Enables (DIP-14).
///
...
/// ```
///
/// Four independent non-inverting buffers, each with its own active-low
/// enable. Enabled (~G low): Y follows A. Disabled (~G high): Y goes to
/// high impedance so several outputs can share one bus.
class Chip74LS125 extends ChipDefinition {
  @override
  String get model => '74LS125';
  ...
  String get functionSummary =>
      '四个独立的三态总线缓冲器（不反相）。每个缓冲器的使能 ~G 为低时 '
      'Y 跟随 A；~G 为高时 Y 呈高阻（highZ），允许多个输出共享同一条'
      '总线。';
  ...
  List<String> get datasheetNotes => const [
        '~G 为低时 Y 跟随 A（不反相）；~G 为高时 Y 呈高阻（highZ）。',
        '使能未知（X）时输出未知；使能为低且数据悬空（Z）时输出未知。',
