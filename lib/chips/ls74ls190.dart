...
/// 74LS190 — Synchronous BCD Decade Up/Down Counter (DIP-16).
///
...
/// 0-9 and U/D high counts down through 9-0. TC goes high at the terminal
/// count; RC is normally high and pulses low while CP, ~CE and TC are all
/// active.
class Chip74LS190 extends ChipDefinition {
  @override
  String get model => '74LS190';

  @override
  String get description => '十进制\n加/减计数器';

  @override
  String get functionSummary => '同步十进制（BCD 8421）加/减计数器，单一 CP 上升沿计数；'
  ...
  List<String> get datasheetNotes => const [
        ...
        '仅当 ~CE 为低时在 CP 上升沿计数：U/D 为低时 0~9 循环加，为高时 0→9 减。',
        'TC 在加计数到 9 或减计数到 0 时为高；RC 平时为高，仅当 ~CE 低、TC 高且 CP 高时为低。',
      ];
  ...
  Map<int, SignalState> evaluate(
    ...
    if (pl == SignalState.low) {
      ...
      if (upDown == SignalState.low) {
        _count(state, up: true, modulus: 10);
      } else if (upDown == SignalState.high) {
        _count(state, up: false, modulus: 10);
      }
    ...
    final tc = _terminalCount(upDown: upDown, value: value, maxValue: 9);
