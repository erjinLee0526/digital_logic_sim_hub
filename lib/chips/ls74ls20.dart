...
/// 74LS20 - Dual 4-Input NAND Gate (DIP-14).
///
...
///   Power: VCC(14), GND(7)
class Chip74LS20 extends ChipDefinition {
  @override
  String get model => '74LS20';
  ...
  String get functionSummary =>
      '内含两个独立的 4 输入与非门：只有当四个输入都为高电平时，输出才为'
      '低电平；任意输入为低电平，输出即为高电平。';

  @override
  int get propagationDelayPs => 15000; // max 15ns (TI datasheet)
  ...
  Map<int, SignalState> evaluate(
    ...
    return {
      ...
      6: _nand4(_val(1, inputStates), _val(2, inputStates),
          _val(4, inputStates), _val(5, inputStates)),
      // Gate 2: pins 9,10,12,13 -> pin 8
      8: _nand4(_val(9, inputStates), _val(10, inputStates),
  ...
  static SignalState _nand4(
      SignalState a, SignalState b, SignalState c, SignalState d) {
    return SignalState.nand4(a, b, c, d);
  }
}
