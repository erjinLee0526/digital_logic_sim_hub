...
/// 74LS193 — Synchronous 4-Bit Binary Up/Down Counter with Separate
/// Up/Down Clocks (DIP-16).
///
...
/// high, a rising edge of CPU counts up while CPD is high, and a rising edge
/// of CPD counts down while CPU is high.
class Chip74LS193 extends ChipDefinition {
  @override
  String get model => '74LS193';

  @override
  String get description => '4 位二进制\n加/减计数器';

  @override
  String get functionSummary => '同步 4 位二进制加/减计数器，带独立的加计数时钟 CPU 与'
  ...
  List<String> get datasheetNotes => const [
        ...
        'CPD 为高时 CPU 上升沿加计数（0~15 循环），CPU 为高时 CPD 上升沿减计数（0→15）。',
        '计数到 15 且 CPU 为低时 TCU 为低，计数到 0 且 CPD 为低时 TCD 为低，其余为高。',
      ];
  ...
  static void _count(Map<String, SignalState> state, {required bool up}) {
    ...
    final next = up ? (value + 1) & 0xf : (value == 0 ? 15 : value - 1);
    _encode(next, state);
  }
  ...
  /// TCU = ~(/CPU * Q0 * Q1 * Q2 * Q3): low at count 15 while CPU is low.
  static SignalState _carry({
    ...
    if (cpu == SignalState.low) {
      ...
      return value == 15 ? SignalState.low : SignalState.high;
    }
