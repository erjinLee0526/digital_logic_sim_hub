...
/// 74LS136 - Quad 2-Input XOR Gate, Open-Collector Outputs (DIP-14).
///
...
class Chip74LS136 extends ChipDefinition {
  @override
  String get model => '74LS136';
  ...
  String get functionSummary =>
      '内含四个独立的 2 输入异或门，输出为集电极开路结构：两个输入电平'
      '相同时输出低电平，不同时输出悬空，需外接上拉电阻才能获得高电平。';
  ...
  Map<int, SignalState> evaluate(
    ...
    return {
      ...
      3: _oc(_xor(_val(1, inputStates), _val(2, inputStates))),
      // Gate 2: pins 4,5 -> pin 6
      6: _oc(_xor(_val(4, inputStates), _val(5, inputStates))),
      // Gate 3: pins 9,10 -> pin 8
      8: _oc(_xor(_val(9, inputStates), _val(10, inputStates))),
      // Gate 4: pins 12,13 -> pin 11
      11: _oc(_xor(_val(12, inputStates), _val(13, inputStates))),
    };
  }
