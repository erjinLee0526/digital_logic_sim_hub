...
/// 74LS08 - Quad 2-Input AND Gate (DIP-14).
///
...
///   Power: VCC(14), GND(7)
class Chip74LS08 extends ChipDefinition {
  @override
  String get model => '74LS08';
  ...
  String get functionSummary =>
      '内含四个独立的 2 输入与门：只有当两个输入都为高电平时，输出才为'
      '高电平；其余情况输出均为低电平。';
  ...
  Map<int, SignalState> evaluate(
    ...
    return {
      ...
      3: _and(_val(1, inputStates), _val(2, inputStates)),
      // Gate 2: pins 4,5 -> pin 6
      6: _and(_val(4, inputStates), _val(5, inputStates)),
      // Gate 3: pins 9,10 -> pin 8
      8: _and(_val(9, inputStates), _val(10, inputStates)),
      // Gate 4: pins 12,13 -> pin 11
      11: _and(_val(12, inputStates), _val(13, inputStates)),
    };
  }
  ...
  static SignalState _and(SignalState a, SignalState b) {
    return SignalState.and(a, b);
  }
}
