...
import 'package:digital_logic_sim/chips/ls74ls03.dart';
import 'package:digital_logic_sim/models/signal_state.dart';

void main() {
  late Chip74LS03 chip;

  setUp(() => chip = Chip74LS03());
  ...
  group('Chip74LS03', () {
    ...
    test('each open-collector gate follows the NAND truth table', () {
      // NAND logic 1 becomes highZ (floating open-collector output).
      const cases = [
        (SignalState.low, SignalState.low, SignalState.highZ),
    ...
    test('low input dominates unknown and floats the output', () {
      expect(
    ...
    test('the four gates are independent', () {
      final result = chip.evaluate({
        1: SignalState.low,
        2: SignalState.low, // -> 1Y highZ
        4: SignalState.high,
        5: SignalState.high, // -> 2Y low
        9: SignalState.high,
        10: SignalState.high, // -> 3Y low
        12: SignalState.low,
        13: SignalState.high, // -> 4Y highZ
      });
