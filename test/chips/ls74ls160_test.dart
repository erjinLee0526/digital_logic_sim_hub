...
import 'package:digital_logic_sim/chips/ls74ls160.dart';
import 'package:digital_logic_sim/models/signal_state.dart';

void main() {
  late Chip74LS160 chip;
  ...
  group('Chip74LS160', () {
    ...
    test('counts modulo 10 when ENP and ENT are both high', () {
      final state = Map<String, SignalState>.of(chip.initialState);
      for (var i = 0; i < 4; i++) {
      ...
      // Jump to state 9 and confirm the next count wraps to 0.
    ...
    test('RCO is high only in state 9 with ENT high', () {
      final state = Map<String, SignalState>.of(chip.initialState);
      state['q0'] = SignalState.high;
      state['q1'] = SignalState.low;
      state['q2'] = SignalState.low;
      ...
      final atNine = chip.evaluate(
        {1: SignalState.high, 10: SignalState.high},
        internalState: state,
      );
      expect(atNine[15], SignalState.high);

      state['q0'] = SignalState.high;
      state['q1'] = SignalState.high;
      state['q2'] = SignalState.high;
      state['q3'] = SignalState.high;
      final atFifteen = chip.evaluate(
        {1: SignalState.high, 10: SignalState.high},
        internalState: state,
      );
      expect(atFifteen[15], SignalState.low);
