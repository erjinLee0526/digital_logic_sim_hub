...
import 'package:digital_logic_sim/chips/ls74ls192.dart';
import 'package:digital_logic_sim/models/signal_state.dart';

void main() {
  late Chip74LS192 chip;
  ...
  group('Chip74LS192', () {
      ...
      final result = chip.evaluate(
        {
          ...
          10: SignalState.high,
          9: SignalState.high,
          5: SignalState.high, // CPU rising from unknown
          4: SignalState.high, // CPD rising from unknown
        },
        internalState: state,
      );
    ...
    test('counts up on CPU rising edges while CPD is high', () {
      ...
      expect(result[3], SignalState.high, reason: '8 + 1 = 9');
      expect(result[2], SignalState.low);
      expect(result[6], SignalState.low);
      expect(result[7], SignalState.high);
    ...
    test('TCU and TCD track terminal counts and clock levels', () {
      ...
      // Load 9 (1001): TCU low while CPU is low.
      var result = chip.evaluate(
        {
          ...base,
          11: SignalState.low,
      ...
      expect(result[12], SignalState.low, reason: 'TCU low at 9 with CPU low');
      expect(result[13], SignalState.high, reason: 'TCD high elsewhere');

      // Raise CPU: carry window closes.
      result = chip.evaluate(
        {...base, 5: SignalState.high},
        internalState: state,
      );
      expect(result[12], SignalState.high, reason: 'TCU high while CPU high');
      expect(result[13], SignalState.high, reason: 'TCD high while CPD high');

      // Load 0 (0000) with CPD low: TCD low while CPD is low.
      result = chip.evaluate(
        {
          ...base,
          11: SignalState.low,
      ...
      expect(result[12], SignalState.high, reason: 'TCU high away from 9');
