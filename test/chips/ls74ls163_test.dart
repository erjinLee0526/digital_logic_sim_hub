...
import 'package:digital_logic_sim/chips/ls74ls163.dart';
import 'package:digital_logic_sim/models/signal_state.dart';

void main() {
  late Chip74LS163 chip;
  ...
  group('Chip74LS163', () {
    ...
    test('synchronous clear only takes effect on a rising CLK edge', () {
      ...
      // ~CLR low while CLK is already high: no edge, so no change.
      final noEdge = chip.evaluate(
        {
          1: SignalState.low,
          2: SignalState.high,
          7: SignalState.high,
      ...
      expect(noEdge[11], SignalState.high);

      // Now create a rising edge with ~CLR low.
      state['prev_clk'] = SignalState.low;
      final cleared = chip.evaluate(
        {
          1: SignalState.low,
          2: SignalState.high,
          7: SignalState.high,
      ...
      expect(cleared[11], SignalState.low);
      expect(cleared[12], SignalState.low);
      expect(cleared[13], SignalState.low);
      expect(cleared[14], SignalState.low);
      expect(cleared[15], SignalState.low);
    });

    test('clear has priority over load on the rising edge', () {
      final state = Map<String, SignalState>.of(chip.initialState);
      state['prev_clk'] = SignalState.low;
    ...
    test('counts modulo 16 when ENP and ENT are both high', () {
      final state = Map<String, SignalState>.of(chip.initialState);
      for (var i = 0; i < 4; i++) {
      ...
      // Jump to state 15 and confirm the next count wraps to 0.
      for (var i = 0; i < 4; i++) {
        state['q$i'] = SignalState.high;
      }
      state['prev_clk'] = SignalState.low;
      final wrapped = pulse();
      expect(wrapped[11], SignalState.low);
      expect(wrapped[12], SignalState.low);
      expect(wrapped[13], SignalState.low);
      expect(wrapped[14], SignalState.low);
    });
